// Веб-версия каталога калькуляторов
// Без зависимостей от Isar (чек-листы временно отключены)
...
/// Веб-версия каталога калькуляторов.
/// Не содержит зависимостей от Isar-моделей (чек-листы, проекты).
class ModernCalculatorCatalogScreenWeb extends ConsumerStatefulWidget {
  const ModernCalculatorCatalogScreenWeb({super.key});

  @override
  ConsumerState<ModernCalculatorCatalogScreenWeb> createState() =>
      _ModernCalculatorCatalogScreenWebState();
}

class _ModernCalculatorCatalogScreenWebState
    extends ConsumerState<ModernCalculatorCatalogScreenWeb> {
  ...
  Widget build(BuildContext context) {
    ...
    // Адаптивная ширина для веба
    final isWideScreen = kIsWeb && width >= CalculatorDesignSystem.breakpointDesktop;
    const maxCatalogWidth = 1200.0;
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          // Чек-листы временно отключены на вебе
          // IconButton(
          //   onPressed: () async {
        ...
        child: Center(
          child: ConstrainedBox(
            constraints: BoxConstraints(
              maxWidth: isWideScreen ? maxCatalogWidth : double.infinity,
            ),
