...
bool hasCanonicalTileInputs(Map<String, double> inputs) {
  return inputs.containsKey('tileWidthCm') ||
      inputs.containsKey('tileHeightCm');
}

Map<String, double> normalizeLegacyTileInputs(Map<String, double> inputs) {
  final normalized = Map<String, double>.from(inputs);
  final tileSize = (inputs['tileSize'] ?? 60).toDouble();
  if (!normalized.containsKey('tileWidthCm') ||
      !normalized.containsKey('tileHeightCm')) {
    double tileWidthCm;
    double tileHeightCm;
    if (tileSize == 0) {
  ...
  if (!normalized.containsKey('layoutPattern')) {
    normalized['layoutPattern'] = (inputs['layoutPattern'] ?? 1)
        .round()
        .clamp(1, 4)
        .toDouble();
  }
  if (!normalized.containsKey('roomComplexity')) {
    normalized['roomComplexity'] = (inputs['roomComplexity'] ?? 1)
        .round()
        .clamp(1, 3)
        .toDouble();
  }
...
Map<String, double> _resolveArea(SpecReader spec, Map<String, double> inputs) {
  ...
  if (inputMode == 0) {
    final length = math
        .max(0.5, inputs['length'] ?? defaultFor(spec, 'length', 4))
        .toDouble();
    final width = math
        .max(0.5, inputs['width'] ?? defaultFor(spec, 'width', 3))
        .toDouble();
  ...
  return {
    'inputMode': 1.0,
    'area': roundValue(
      math.max(1, inputs['area'] ?? defaultFor(spec, 'area', 12)).toDouble(),
      3,
    ),
  };
}
...
Map<String, dynamic> _resolveLayout(
  ...
  final layoutId =
      (inputs['layoutPattern'] ?? defaultFor(spec, 'layoutPattern', 1))
          .round()
          .clamp(1, 4);
  return spec
      .normativeList('layouts')
      .firstWhere(
        (item) => (item['id'] as num).toInt() == layoutId,
        orElse: () => spec.normativeList('layouts').first,
      );
}

Map<String, dynamic> _resolveRoomComplexity(
  SpecReader spec,
  Map<String, double> inputs,
) {
  final complexityId =
      (inputs['roomComplexity'] ?? defaultFor(spec, 'roomComplexity', 1))
          .round()
          .clamp(1, 3);
  return spec
      .normativeList('room_complexities')
      .firstWhere(
        (item) => (item['id'] as num).toInt() == complexityId,
        orElse: () => spec.normativeList('room_complexities').first,
      );
}

double _resolveTileSizeAdjustment(SpecReader spec, double averageTileSizeCm) {
  if (averageTileSizeCm >
      spec.warningRule<num>('large_tile_warning_threshold_cm').toDouble()) {
...
double _resolveGlueRate(SpecReader spec, double averageTileSizeCm) {
  if (averageTileSizeCm < 20) {
    return spec.materialRule<num>('glue_kg_per_m2_small').toDouble();
  }
  if (averageTileSizeCm < 40) {
    return spec.materialRule<num>('glue_kg_per_m2_medium').toDouble();
  }
  if (averageTileSizeCm <= 60) {
    return spec.materialRule<num>('glue_kg_per_m2_large').toDouble();
  }
  return spec.materialRule<num>('glue_kg_per_m2_xl').toDouble();
}

double _resolveGroutDepth(
  double averageTileSizeCm,
  double requestedGroutDepth,
) {
...
CanonicalCalculatorContractResult calculateCanonicalTile(
  ...
  final normalized = hasCanonicalTileInputs(inputs)
      ? Map<String, double>.from(inputs)
      : normalizeLegacyTileInputs(inputs);
  final areaInfo = _resolveArea(spec, normalized);
  final area = areaInfo['area']!;
  final tileWidthCm =
      (normalized['tileWidthCm'] ?? defaultFor(spec, 'tileWidthCm', 30))
          .clamp(5, 200)
          .toDouble();
  final tileHeightCm =
      (normalized['tileHeightCm'] ?? defaultFor(spec, 'tileHeightCm', 30))
          .clamp(5, 200)
          .toDouble();
  final layout = _resolveLayout(spec, normalized);
  final roomComplexity = _resolveRoomComplexity(spec, normalized);
  final jointWidthMm =
      (normalized['jointWidth'] ?? defaultFor(spec, 'jointWidth', 3))
          .clamp(1, 10)
          .toDouble();
  ...
  final wastePercent = roundValue(
    (layout['waste_percent'] as num).toDouble() +
        (roomComplexity['waste_bonus_percent'] as num).toDouble() +
        sizeAdjustment,
    3,
  );
  ...
  final requestedPackAreaM2 =
      (normalized['packArea'] ??
              defaultFor(
                spec,
                'packArea',
                spec.packagingRule<num>('tile_package_area_m2').toDouble(),
              ))
          .clamp(0.1, 20)
          .toDouble();
  ...
  final baseExactNeed = tileAreaM2 > 0
      ? roundValue(
          (area / tileAreaM2) * (1 + wastePercent / 100) * tilePrimaryMult,
          6,
        )
      : 0.0;
  final groutDepthMm = _resolveGroutDepth(
    averageTileSizeCm,
    (normalized['groutDepth'] ?? 0).toDouble(),
  );
  final tileWidthM = tileWidthCm / 100;
  final tileHeightM = tileHeightCm / 100;
  final jointsLength = (1 / tileWidthM) + (1 / tileHeightM);
  final groutKg = roundValue(
    area *
        jointsLength *
        (jointWidthMm / 1000) *
        (groutDepthMm / 1000) *
        spec.materialRule<num>('grout_density_kg_per_m3').toDouble() *
        spec.materialRule<num>('grout_loss_factor').toDouble() *
        accuracyPrimaryMultiplier('grout', accuracyMode),
    6,
  );
  final glueRate = _resolveGlueRate(spec, averageTileSizeCm);
  final glueKg = roundValue(
    area * glueRate * accuracyPrimaryMultiplier('tile_adhesive', accuracyMode),
    6,
  );
  ...
  for (final scenarioName in scenarioNames) {
    ...
    final packageSize = tilesPerPackage.toDouble();
    final packageCount = exactNeed > 0 ? (exactNeed / packageSize).ceil() : 0;
    final purchaseQuantity = roundValue(packageCount * packageSize, 6);
    final packageLabel = 'tile-box-${packageSize.toInt()}';
  ...
  final glueBags = glueKg > 0
      ? math.max(
          1,
          (glueKg / spec.packagingRule<num>('glue_bag_kg').toDouble()).ceil(),
        )
      : 0;
  final groutBags = groutKg > 0
      ? math.max(
          1,
          (groutKg / spec.packagingRule<num>('grout_bag_kg').toDouble()).ceil(),
        )
      : 0;
  final primerCans = primerLiters > 0
      ? math.max(
          1,
          (primerLiters / spec.packagingRule<num>('primer_can_l').toDouble())
              .ceil(),
        )
      : 0;
  final accessoriesMult = accuracyAccessoriesMultiplier('tile', accuracyMode);
  final crossesNeeded =
      (recScenario.purchaseQuantity *
              spec.materialRule<num>('crosses_reserve_factor').toDouble() *
              accessoriesMult)
          .ceil();
  final svpPackages =
      averageTileSizeCm >= spec.materialRule<num>('svp_threshold_cm').toDouble()
      ? math.max(
          1,
          (crossesNeeded / spec.packagingRule<num>('svp_pack_size').toDouble())
              .ceil(),
        )
      : 0;
  final siliconeTubes = math.max(
    1,
    (area / spec.materialRule<num>('silicone_tube_area_m2').toDouble()).ceil(),
  );
  ...
  if (baseExactNeed <
    ...
    warnings.add(
      'При укладке меньше 5 плиток процент отходов может быть выше расчётного',
    );
  }
  if ((layout['id'] as num).toInt() == 2) {
    warnings.add(
      'Диагональная укладка требует большего запаса и аккуратной подрезки',
    );
  }
  if (averageTileSizeCm >
      spec.warningRule<num>('large_tile_warning_threshold_cm').toDouble()) {
    warnings.add(
      'Крупный формат требует двойного нанесения клея и более ровного основания',
    );
  }
  if ((layout['id'] as num).toInt() == 4 &&
      area > spec.warningRule<num>('herringbone_large_area_m2').toDouble()) {
    warnings.add(
      'Укладка ёлочкой на большой площади сильно увеличивает отходы и требования к раскладке',
    );
  }
  ...
  final materials = <CanonicalMaterialResult>[
    CanonicalMaterialResult(
      name:
          'Плитка ${(tileWidthCm * 10).round()}×${(tileHeightCm * 10).round()} мм',
    ...
    CanonicalMaterialResult(
      name:
          'Плиточный клей для керамической плитки (${spec.packagingRule<num>('glue_bag_kg').toInt()} кг)',
      quantity: glueKg,
      unit: 'кг',
      withReserve: glueBags * spec.packagingRule<num>('glue_bag_kg').toDouble(),
      purchaseQty:
          (glueBags * spec.packagingRule<num>('glue_bag_kg').toDouble())
              .toDouble(),
      packageInfo: {
        'count': glueBags,
        'size': spec.packagingRule<num>('glue_bag_kg').toDouble(),
        'packageUnit': 'мешков',
      },
      category: 'Клей',
    ),
    CanonicalMaterialResult(
      name:
          'Затирка цементная для шва ${jointWidthMm.toInt()} мм (${spec.packagingRule<num>('grout_bag_kg').toInt()} кг)',
      quantity: groutKg,
      unit: 'кг',
      withReserve:
          groutBags * spec.packagingRule<num>('grout_bag_kg').toDouble(),
      purchaseQty:
          (groutBags * spec.packagingRule<num>('grout_bag_kg').toDouble())
              .toDouble(),
      packageInfo: {
        'count': groutBags,
        'size': spec.packagingRule<num>('grout_bag_kg').toDouble(),
        'packageUnit': 'мешков',
      },
      category: 'Затирка',
    ),
    CanonicalMaterialResult(
      ...
      packageInfo: {
        'count': primerCans,
        'size': spec.packagingRule<num>('primer_can_l').toDouble(),
        'packageUnit': 'канистр',
      },
      category: 'Подготовка',
    ),
  ...
  if (averageTileSizeCm >=
      spec.materialRule<num>('svp_threshold_cm').toDouble()) {
    materials.add(
      CanonicalMaterialResult(
    ...
    materials.add(
      CanonicalMaterialResult(
        name: 'Крестики для плитки ${jointWidthMm.toInt()} мм',
