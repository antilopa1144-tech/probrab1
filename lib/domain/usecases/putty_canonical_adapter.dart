...
double _resolveWorkArea(SpecReader spec, Map<String, double> inputs) {
  ...
  if (inputMode == 0) {
    ...
    final surfaceMode = (inputs['surface'] ?? defaultFor(spec, 'surface', 0))
        .round();
...
double _resolveBagWeight(SpecReader spec, Map<String, double> inputs) {
  final defaultSize = spec
      .packagingRule<num>('default_package_size', 25)
      .toDouble();
...
Map<String, dynamic> _resolveQualityProfile(
  SpecReader spec,
  Map<String, double> inputs,
) {
  final qualityClass =
      (inputs['qualityClass'] ?? defaultFor(spec, 'qualityClass', 0))
          .round()
          .clamp(0, 3);
  ...
  return profiles.isNotEmpty
      ? profiles.first
      : {'id': 0, 'key': 'default', 'components': <String, dynamic>{}};
}
...
int _resolveComponentLayers(
  ...
  final legacyLayers = (inputs['layers'] ?? defaultFor(spec, 'layers', 0))
      .round()
      .clamp(0, 5);
  final overrideKey = componentKey == 'start' ? 'startLayers' : 'finishLayers';
  final explicitLayers =
      (inputs[overrideKey] ?? defaultFor(spec, overrideKey, 0)).round().clamp(
        0,
        5,
      );
...
List<_ResolvedPuttyComponent> _resolveComponents(
  ...
  final profileComponents =
      (qualityProfile['components'] as Map<String, dynamic>?) ?? {};
  return components
      .where(
        (component) => ((component['enabled_for_putty_types'] as List?) ?? [])
            .contains(puttyType),
      )
      .map((component) {
        final componentKey = component['key'] as String;
        final profileComponent =
            profileComponents[componentKey] as Map<String, dynamic>?;
        final fallbackLayers =
            (profileComponent?['default_layers'] as num?)?.toInt() ??
            (component['thickness_mm'] as num).round();
        return _ResolvedPuttyComponent(
          component: component,
          consumptionPerLayer:
              (profileComponent?['consumption_kg_per_m2_layer'] as num?)
                  ?.toDouble() ??
              (component['consumption_kg_per_m2_mm'] as num).toDouble(),
          layers: _resolveComponentLayers(
            spec,
            inputs,
            componentKey,
            fallbackLayers,
...
double _findResolvedLayers(
  List<_ResolvedPuttyComponent> components,
  String key,
) {
...
CanonicalCalculatorContractResult calculateCanonicalPutty(
  ...
  final puttyType = (inputs['puttyType'] ?? defaultFor(spec, 'puttyType', 0))
      .round()
      .clamp(0, 2);
  final bagWeight = _resolveBagWeight(spec, inputs);
  final qualityProfile = _resolveQualityProfile(spec, inputs);
  final resolvedComponents = _resolveComponents(
    spec,
    inputs,
    puttyType,
    qualityProfile,
  ...
  final accuracyMult = accuracyPrimaryMultiplier('putty', accuracyMode);
  final accessoriesMult = accuracyAccessoriesMultiplier('putty', accuracyMode);
  if (accuracyMode.name == 'professional' &&
      (puttyType == 0 || puttyType == 1)) {
    for (final component in resolvedComponents) {
      if (component.component['key'] == 'finish') component.layers += 1;
    }
  }
  ...
  for (final scenarioName in scenarioNames) {
    ...
    final keyFactors = buildKeyFactors(
      spec.enabledFactors,
      _factorTable,
      scenarioName,
    );
    final componentScenarios = resolvedComponents
        .map((resolved) {
          final exactNeed = roundValue(
            workArea *
                resolved.layers *
                resolved.consumptionPerLayer *
                accuracyMult *
                multiplier,
            3,
          );
    ...
    final exactNeed = roundValue(
      componentScenarios.fold(
        0.0,
        (sum, component) => sum + component.exactNeed,
      ),
      3,
    );
    final purchaseQuantity = roundValue(
      componentScenarios.fold(
        0.0,
        (sum, component) => sum + component.purchaseQuantity,
      ),
      3,
    );
    final leftover = roundValue(
      componentScenarios.fold(
        0.0,
        (sum, component) => sum + component.leftover,
      ),
      3,
    );
    ...
    scenarios[scenarioName] = CanonicalScenarioResult(
      ...
      assumptions: [
        ...
        for (final component in componentScenarios)
          'component:${component.component['key']}:layers:${component.exactNeed > 0
              ? component.component['key'] == "start"
                    ? _findResolvedLayers(resolvedComponents, "start").toInt()
                    : _findResolvedLayers(resolvedComponents, "finish").toInt()
              : 0}',
      ],
      keyFactors: keyFactors,
  ...
  final materials = <CanonicalMaterialResult>[
    for (final componentScenario in recComponentScenarios)
      CanonicalMaterialResult(
        name:
            'Шпаклёвка ${(componentScenario.component['label'] as String).toLowerCase()} (мешки ${bagWeight.toInt()} кг)',
        quantity: roundValue(componentScenario.exactNeed / bagWeight, 3),
        unit: 'мешков',
        withReserve: (componentScenario.purchaseQuantity / bagWeight)
            .ceilToDouble(),
        purchaseQty: (componentScenario.purchaseQuantity / bagWeight)
            .ceil()
            .toDouble(),
  ...
  if (puttyType == 1 || puttyType == 2) {
    final serpyankaMeters =
        workArea *
        spec.materialRule<num>('serpyanka_linear_m_per_m2').toDouble() *
        spec.materialRule<num>('serpyanka_reserve_factor').toDouble() *
        accessoriesMult;
    materials.add(
      CanonicalMaterialResult(
        name:
            'Серпянка (лента армировочная 45 мм, рулон ${spec.materialRule<num>('serpyanka_roll_length_m').toInt()} м)',
        quantity: roundValue(
          workArea *
              spec.materialRule<num>('serpyanka_linear_m_per_m2').toDouble() *
              accessoriesMult,
        ...
        purchaseQty:
            (serpyankaMeters /
                    spec
                        .materialRule<num>('serpyanka_roll_length_m')
                        .toDouble())
                .ceil()
                .toDouble(),
  ...
  final primerLiters =
      workArea *
      spec.materialRule<num>('primer_l_per_m2_per_coat').toDouble() *
      primerCoats *
      accessoriesMult;
  materials.add(
    CanonicalMaterialResult(
  ...
  if (spec
      .materialRule<List>('sandpaper_enabled_for_putty_types')
      .contains(puttyType)) {
    final sandpaperSheets =
        (workArea / spec.materialRule<num>('sandpaper_m2_per_sheet').toDouble())
            .ceil();
    final purchaseQty =
        (sandpaperSheets *
                spec.materialRule<num>('sandpaper_reserve_factor').toDouble())
            .ceil();
    materials.add(
      CanonicalMaterialResult(
  ...
  if (workArea >
      spec.warningRule<num>('mechanized_area_threshold_m2').toDouble()) {
    warnings.add(
      'Для больших площадей рекомендуется нанесение шпаклёвки механизированным методом',
    );
  }
