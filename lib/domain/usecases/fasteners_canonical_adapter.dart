...
CanonicalCalculatorContractResult calculateCanonicalFasteners(
  ...
  final materialType =
      (inputs['materialType'] ?? defaultFor(spec, 'materialType', 0))
          .round()
          .clamp(0, 3);
  final sheetCount = math.max(
    1,
    math.min(
      200,
      (inputs['sheetCount'] ?? defaultFor(spec, 'sheetCount', 10)).round(),
    ),
  );
  final fastenerStep = math.max(
    150,
    math.min(
      300,
      (inputs['fastenerStep'] ?? defaultFor(spec, 'fastenerStep', 200)).round(),
    ),
  );
  final withFrameScrews =
      (inputs['withFrameScrews'] ?? defaultFor(spec, 'withFrameScrews', 0))
              .round() ==
          1
      ? 1
      : 0;
  final withDubels =
      (inputs['withDubels'] ?? defaultFor(spec, 'withDubels', 0)).round() == 1
      ? 1
      : 0;
  final accuracyMode = parseAccuracyMode(inputs);
  final accuracyMult = accuracyPrimaryMultiplier('fasteners', accuracyMode);
  ...
  // Formulas
  final baseStep =
      (spec.materialRule<Map>('base_step')['$materialType'] as num?)
          ?.toDouble() ??
      (spec.materialRule<Map>('base_step')['0'] as num?)?.toDouble() ??
      0.0;
  final stepCoeff = baseStep / fastenerStep;
  final baseScrews =
      (spec.materialRule<Map>('screws_per_unit')['$materialType'] as num?)
          ?.toDouble() ??
      (spec.materialRule<Map>('screws_per_unit')['0'] as num?)?.toDouble() ??
      0.0;
  final screwsPerUnit = (baseScrews * stepCoeff).ceil();
  var totalScrews =
      (sheetCount *
              screwsPerUnit *
              spec.materialRule<num>('screw_reserve').toDouble() *
              accuracyMult)
          .ceil();

  // Klaimers for paneling
  final klaimers = materialType == 3
      ? (totalScrews * spec.materialRule<num>('klaymer_multiplier').toDouble())
            .ceil()
      : 0;
  if (materialType == 3) {
  ...
  final frameScrews = withFrameScrews == 1
      ? (sheetCount *
                spec.materialRule<num>('frame_screws_per_unit').toDouble() *
                spec.materialRule<num>('frame_screw_reserve').toDouble())
            .ceil()
      : 0;
  ...
  final dubels = withDubels == 1
      ? (sheetCount *
                2 /
                spec.materialRule<num>('dubel_step').toDouble() *
                spec.materialRule<num>('dubel_reserve').toDouble())
            .ceil()
      : 0;
  ...
  final bits =
      (totalScrews /
              spec.materialRule<num>('bits_per_screws').toDouble() *
              accessoriesMult)
          .ceil();
  ...
  const screwLabels = {
    0: 'Чёрные саморезы для гипсокартона по металлу 3,5×25 мм, под крестовую биту PH2',
    1: 'Саморезы по дереву 3,5×35 мм, под крестовую биту PH2',
    2: 'Кровельные саморезы 4,8×35 мм с уплотнительной шайбой из EPDM-резины',
    3: 'Кляймеры для вагонки №3–№5',
  };
  final screwLabel = screwLabels[materialType]!;
  ...
  final useKg = perKg > 0;
  final screwQtyKg = useKg
      ? (recScenario.exactNeed / perKg * 10).ceil() / 10
      : 0.0;
  ...
  final materials = <CanonicalMaterialResult>[
    CanonicalMaterialResult(
      ...
      purchaseQty: useKg
          ? screwQtyKg.ceil().toDouble()
          : recScenario.exactNeed.ceil().toDouble(),
      category: 'Крепёж',
    ),
  ];
  ...
  materials.add(
    CanonicalMaterialResult(
      name: 'Крестовая бита PH2 для шуруповёрта',
