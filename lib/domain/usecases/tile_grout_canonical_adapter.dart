...
CanonicalCalculatorContractResult calculateCanonicalTileGrout(
  ...
  final tileWidth = math.max(
    ...
    math.min(
      1200,
      (inputs['tileWidth'] ?? defaultFor(spec, 'tileWidth', 300)).round(),
    ),
  );
  final tileHeight = math.max(
    ...
    math.min(
      1200,
      (inputs['tileHeight'] ?? defaultFor(spec, 'tileHeight', 300)).round(),
    ),
  );
  final tileThickness = math.max(
    6,
    math.min(
      25,
      (inputs['tileThickness'] ?? defaultFor(spec, 'tileThickness', 8)).round(),
    ),
  );
  final jointWidth = math.max(
    1,
    math.min(
      20,
      (inputs['jointWidth'] ?? defaultFor(spec, 'jointWidth', 3)).round(),
    ),
  );
  final groutType = (inputs['groutType'] ?? defaultFor(spec, 'groutType', 0))
      .round()
      .clamp(0, 2);
  final bagSizeRaw = inputs['bagSize'] ?? defaultFor(spec, 'bagSize', 2);
  final bagSize = [1.0, 2.0, 5.0].contains(bagSizeRaw) ? bagSizeRaw : 2.0;
  final groutDepth = tileThickness * 0.75;
  ...
  final jointVolPerM2 =
      jointLenPerM2 * (jointWidth / 1000) * (groutDepth / 1000) * 1000;

  // Density in kg/L
  final density =
      ((spec.materialRule<Map>('grout_density')['$groutType'] as num?)
              ?.toDouble() ??
          (spec.materialRule<Map>('grout_density')['0'] as num?)?.toDouble() ??
          0.0) /
      1000;

  final kgPerM2 = jointVolPerM2 * density;
  final totalKg =
      area * kgPerM2 * spec.materialRule<num>('grout_reserve').toDouble();
  ...
  final accuracyMult = accuracyPrimaryMultiplier('grout', accuracyMode);
  ...
  if (groutType == 1) {
    warnings.add(
      'Эпоксидная затирка требует быстрого нанесения — готовьте небольшими порциями',
    );
  }
  if (jointWidth >=
      spec.warningRule<num>('wide_joint_threshold_mm').toDouble()) {
  ...
  final materials = <CanonicalMaterialResult>[
    CanonicalMaterialResult(
      ...
      packageInfo: {
        'count': (recScenario.purchaseQuantity / bagSize).ceil(),
        'size': bagSize,
        'packageUnit': 'мешков',
      },
      category: 'Основное',
  ...
  return CanonicalCalculatorContractResult(
    ...
    totals: {
      ...
      'groutDepth': groutDepth.toDouble(),
