...
class CalculateTileGlue extends BaseCalculator {
  ...
  CalculatorResult calculate(
    ...
    // Корректировка по размеру плитки (размер гребёнки зависит от плитки):
    // - мозаика <10 см: гребёнка 3-4 мм, тонкий слой
    // - мелкая 10-20 см: гребёнка 6 мм
    // - средняя 20-40 см: гребёнка 6-8 мм (базовая)
    // - крупная 40-60 см: гребёнка 8-10 мм, больше клея
    // - крупноформат >60 см: гребёнка 10-12 мм + нанесение на плитку и основание
    if (tileSize < 10) {
      baseConsumption *= 0.7; // мозаика — тонкий слой, гребёнка 3-4 мм
    } else if (tileSize < 20) {
      baseConsumption *= 0.85; // мелкая — гребёнка 6 мм
    } else if (tileSize > 60) {
      baseConsumption *= 1.4; // крупноформат — гребёнка 10-12 мм, двойное нанесение
    } else if (tileSize > 40) {
      baseConsumption *= 1.2; // крупная — гребёнка 8-10 мм
    }
