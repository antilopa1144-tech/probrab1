...
class CalculateWarmFloor extends BaseCalculator {
  ...
  String? validateInputs(Map<String, double> inputs) {
    ...
    final minPower = getConstantDouble(
      'power_limits',
      'min_power',
      defaultValue: 80.0,
    );
    final maxPower = getConstantDouble(
      'power_limits',
      'max_power',
      defaultValue: 200.0,
    );
    ...
    if (power < minPower || power > maxPower) {
      return rangeMessage('power', minPower, maxPower, unit: 'Вт/м²');
    }
    if (type < 1 || type > 2) return rangeMessage('type', 1, 2);
  ...
  CalculatorResult calculate(
    ...
    switch (roomType) {
      ...
      default: // Пользовательское значение
        ...
        final defaultPower = getConstantDouble(
          'room_power',
          'default',
          defaultValue: 150.0,
        );
    ...
    final defaultUsefulPercent = getConstantDouble(
      'useful_area',
      'default',
      defaultValue: 70.0,
    );
    final minUsefulPercent = getConstantDouble(
      'useful_area',
      'min',
      defaultValue: 50.0,
    );
    final maxUsefulPercent = getConstantDouble(
      'useful_area',
      'max',
      defaultValue: 90.0,
    );
    ...
    if (type == 1) {
      ...
      final cablePowerPerMeter = getConstantDouble(
        'cable_power',
        'standard_cable',
        defaultValue: 18.0,
      ); // Вт/м
    ...
    final montageTapeFactor = getConstantDouble(
      'materials',
      'montage_tape_factor',
      defaultValue: 1.5,
    );
    ...
    final damperTapeMargin = getMarginPercent(
      'margins',
      'damper_tape_margin',
      defaultValue: 5.0,
    );
    ...
    final rcdNeeded = getConstantInt(
      'safety',
      'rcd_per_system',
      defaultValue: 1,
    );
    ...
    final montTapePrice = findPrice(priceList, [
      'montage_tape',
      'mounting_tape',
    ]);
