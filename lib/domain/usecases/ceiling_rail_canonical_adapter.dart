...
CanonicalCalculatorContractResult calculateCanonicalCeilingRail(
  ...
  final area = math.max(
    1.0,
    math.min(200.0, inputs['area'] ?? defaultFor(spec, 'area', 20)),
  );
  final railWidthRaw =
      (inputs['railWidth'] ?? defaultFor(spec, 'railWidth', 100)).round();
  final allowedWidths = [100, 150, 200];
  final railWidth = allowedWidths.contains(railWidthRaw) ? railWidthRaw : 100;
  final railLengthRaw =
      inputs['railLength'] ?? defaultFor(spec, 'railLength', 3.0);
  ...
  final totalRailLen =
      railRows * roomLength * spec.materialRule<num>('rail_reserve').toDouble();
  final railPcs = (totalRailLen / railLength).ceil();

  // T-profile guides
  final guideCount =
      (roomLength / spec.materialRule<num>('t_profile_spacing').toDouble())
          .ceil() +
      1;
  final guideTotal =
      guideCount * roomWidth * spec.materialRule<num>('t_reserve').toDouble();
  final guidePcs =
      (guideTotal / spec.materialRule<num>('t_profile_length').toDouble())
          .ceil();

  // Hangers
  final hangers =
      ((roomWidth / spec.materialRule<num>('hanger_spacing').toDouble()) + 1)
          .ceil() *
      guideCount;

  // Screws & dubels (in kg)
  final screwsPcs =
      (hangers * spec.materialRule<num>('screws_per_hanger').toDouble() +
              railPcs * spec.materialRule<num>('screws_per_rail').toDouble())
          .round();
  ...
  final railLengthLabel = railLength == railLength.roundToDouble()
      ? railLength.toInt().toString()
      : railLength.toString();
  ...
  final materials = <CanonicalMaterialResult>[
    CanonicalMaterialResult(
      name:
          '\u0420\u0435\u0439\u043a\u0430 $railWidth \u043c\u043c \u00d7 $railLengthLabel \u043c',
