// GENERATED FILE - do not edit by hand.
// CalculatorDefinitionV2 entries used by the app.

import '../../../core/enums/calculator_category.dart';
import '../../../core/enums/field_input_type.dart';
import '../../../core/enums/unit_type.dart';
import '../../models/calculator_definition_v2.dart';
import '../../models/calculator_field.dart';
import '../../models/calculator_hint.dart';
import '../calculator_constants.dart';
import '../../usecases/canonical_bridge.dart';
import '../../usecases/facade_brick_canonical_adapter.dart';
import '../../usecases/facade_panels_v3_adapter.dart';
import '../../usecases/fence_canonical_adapter.dart';
import '../../usecases/stairs_canonical_adapter.dart';
import '../../usecases/terrace_canonical_adapter.dart';
// insulation_canonical_adapter.dart — больше не используется (insulation_mineral_wool удалён)

final List<CalculatorDefinitionV2> facadeCalculators = [
  const CalculatorDefinitionV2(
      id: 'exterior_brick',
      titleKey: 'calculator.exterior_brick.title',
      descriptionKey: 'calculator.exterior_brick.description',
      category: CalculatorCategory.exterior,
      subCategoryKey: 'subcategory.brick_facing',
      fields: [
        CalculatorField(
          key: 'area',
          labelKey: 'input.area',
          unitType: UnitType.squareMeters,
          inputType: FieldInputType.slider,
          defaultValue: 50.0,
          minValue: 5.0,
          maxValue: 1000.0,
          step: 1.0,
          iconName: 'square_foot',
          group: 'dimensions',
          required: true,
          order: 1,
        ),
        CalculatorField(
          key: 'thickness',
          labelKey: 'input.thickness',
          unitType: UnitType.meters,
          inputType: FieldInputType.slider,
          defaultValue: 0.5,
          minValue: 0.5,
          maxValue: 1.0,
          step: 1.0,
          iconName: 'width_normal',
          group: 'material',
          required: true,
          order: 2,
        ),
        CalculatorField(
          key: 'windowsArea',
          labelKey: 'input.windowsArea',
          unitType: UnitType.squareMeters,
          inputType: FieldInputType.slider,
          defaultValue: 0.0,
          minValue: 0.0,
          maxValue: 200.0,
          step: 1.0,
          iconName: 'window',
          group: 'openings',
          required: false,
          order: 3,
        ),
        CalculatorField(
          key: 'doorsArea',
          labelKey: 'input.doorsArea',
          unitType: UnitType.squareMeters,
          inputType: FieldInputType.slider,
          defaultValue: 0.0,
          minValue: 0.0,
          maxValue: 50.0,
          step: 1.0,
          iconName: 'door_front',
          group: 'openings',
          required: false,
          order: 4,
        ),
        CalculatorField(
          key: 'perimeter',
          labelKey: 'input.perimeter',
          unitType: UnitType.meters,
          inputType: FieldInputType.slider,
          defaultValue: 20.0,
          minValue: 4.0,
          maxValue: 200.0,
          step: 1.0,
          iconName: 'crop_square',
          group: 'dimensions',
          required: true,
          order: 5,
        ),
        CalculatorField(
          key: 'wallHeight',
          labelKey: 'input.wallHeight',
          unitType: UnitType.meters,
          inputType: FieldInputType.slider,
          defaultValue: 2.5,
          minValue: 2.0,
          maxValue: 15.0,
          step: 1.0,
          iconName: 'height',
          group: 'dimensions',
          required: true,
          order: 6,
        ),
        CalculatorField(
          key: 'workingConditions',
          labelKey: 'input.workingConditions',
          unitType: UnitType.pieces,
          inputType: FieldInputType.select,
          defaultValue: 1.0,
          iconName: 'thermostat',
          group: 'conditions',
          required: false,
          order: 10,
          options: [
            FieldOption(value: 1.0, labelKey: 'input.workingConditions.normal'),
            FieldOption(value: 2.0, labelKey: 'input.workingConditions.windy'),
            FieldOption(value: 3.0, labelKey: 'input.workingConditions.cold'),
            FieldOption(value: 4.0, labelKey: 'input.workingConditions.hot'),
          ],
        ),
      ],
      beforeHints: [
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.ispolzuyte_oblitsovochnyy_kirpich_s'),
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.armiruyte_cherez_kazhdye_5'),
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.proveryayte_vertikalnost_kazhdogo_ryada'),
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.zaschitite_kladku_ot_dozhdya'),
      ],
      afterHints: [
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.ispolzuyte_oblitsovochnyy_kirpich_s'),
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.armiruyte_cherez_kazhdye_5'),
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.proveryayte_vertikalnost_kazhdogo_ryada'),
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.zaschitite_kladku_ot_dozhdya'),
        CalculatorHint(
          type: HintType.warning,
          messageKey: 'hint.brick.cold_weather_warning',
          condition: HintCondition(
            type: HintConditionType.equals,
            fieldKey: 'workingConditions',
            value: 3,
          ),
        ),
      ],
      useCase: CanonicalBridgeUseCase(calculateCanonicalFacadeBrick),
      accentColor: kCalculatorAccentColor,
      complexity: 3,
      popularity: 10,
      tags: [
        'tag.oblitsovochnyy_kirpich',
        'exterior',
        'exterior_brick',
        'tag.naruzhnaya_otdelka',
        'brick',
      ],
    ),
  const CalculatorDefinitionV2(
      id: 'exterior_facade_panels',
      titleKey: 'calculator.exterior_facade_panels.title',
      descriptionKey: 'calculator.exterior_facade_panels.description',
      category: CalculatorCategory.exterior,
      subCategoryKey: 'subcategory.facade_panels',
      fields: [
        CalculatorField(
          key: 'area',
          labelKey: 'input.area',
          unitType: UnitType.squareMeters,
          inputType: FieldInputType.slider,
          defaultValue: 50.0,
          minValue: 5.0,
          maxValue: 1000.0,
          step: 1.0,
          iconName: 'square_foot',
          group: 'dimensions',
          required: true,
          order: 1,
        ),
        CalculatorField(
          key: 'panelWidth',
          labelKey: 'input.panelWidth',
          unitType: UnitType.centimeters,
          inputType: FieldInputType.slider,
          defaultValue: 50.0,
          minValue: 10.0,
          maxValue: 150.0,
          step: 5.0,
          iconName: 'width_normal',
          group: 'material',
          required: true,
          order: 2,
        ),
        CalculatorField(
          key: 'panelHeight',
          labelKey: 'input.panelHeight',
          unitType: UnitType.centimeters,
          inputType: FieldInputType.slider,
          defaultValue: 100.0,
          minValue: 20.0,
          maxValue: 200.0,
          step: 5.0,
          iconName: 'height',
          group: 'material',
          required: true,
          order: 3,
        ),
        CalculatorField(
          key: 'perimeter',
          labelKey: 'input.perimeter',
          unitType: UnitType.meters,
          inputType: FieldInputType.slider,
          defaultValue: 20.0,
          minValue: 4.0,
          maxValue: 200.0,
          step: 1.0,
          iconName: 'crop_square',
          group: 'dimensions',
          required: true,
          order: 4,
        ),
      ],
      beforeHints: [
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.fasadnye_paneli_dolgovechny_i'),
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.ispolzuyte_kachestvennye_krepleniya'),
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.ostavlyayte_zazor_dlya_temperaturnogo'),
      ],
      afterHints: [
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.fasadnye_paneli_dolgovechny_i'),
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.ispolzuyte_kachestvennye_krepleniya'),
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.ostavlyayte_zazor_dlya_temperaturnogo'),
      ],
      useCase: CanonicalBridgeUseCase(calculateCanonicalFacadePanelsV3),
      accentColor: kCalculatorAccentColor,
      complexity: 3,
      popularity: 10,
      tags: [
        'tag.fasadnye_paneli',
        'exterior',
        'facade',
        'exterior_facade_panels',
        'panels',
        'tag.naruzhnaya_otdelka',
      ],
    ),
  const CalculatorDefinitionV2(
      id: 'fence',
      titleKey: 'calculator.fence.title',
      descriptionKey: 'calculator.fence.description',
      category: CalculatorCategory.exterior,
      subCategoryKey: 'subcategory.fences',
      fields: [
        CalculatorField(
          key: 'fenceLength',
          labelKey: 'input.length',
          unitType: UnitType.meters,
          inputType: FieldInputType.slider,
          defaultValue: 50.0,
          minValue: 5.0,
          maxValue: 500.0,
          step: 1.0,
          iconName: 'straighten',
          group: 'dimensions',
          required: true,
          order: 1,
        ),
        CalculatorField(
          key: 'fenceHeight',
          labelKey: 'input.height',
          unitType: UnitType.meters,
          inputType: FieldInputType.slider,
          defaultValue: 2.0,
          minValue: 1.0,
          maxValue: 3.0,
          step: 0.1,
          iconName: 'height',
          group: 'dimensions',
          required: true,
          order: 2,
        ),
        CalculatorField(
          key: 'postStep',
          labelKey: 'input.postSpacing',
          unitType: UnitType.meters,
          inputType: FieldInputType.slider,
          defaultValue: 2.5,
          minValue: 2.0,
          maxValue: 3.0,
          step: 0.1,
          iconName: 'fence',
          group: 'dimensions',
          required: false,
          order: 3,
        ),
        CalculatorField(
          key: 'fenceType',
          labelKey: 'input.fenceType',
          unitType: UnitType.pieces,
          inputType: FieldInputType.select,
          defaultValue: 0.0,
          iconName: 'category',
          group: 'material',
          required: true,
          order: 4,
          options: [
            FieldOption(value: 0.0, labelKey: 'input.fenceType.profsheet'),
            FieldOption(value: 1.0, labelKey: 'input.fenceType.chainlink'),
            FieldOption(value: 2.0, labelKey: 'input.fenceType.picket'),
          ],
        ),
        CalculatorField(
          key: 'gatesCount',
          labelKey: 'input.gates',
          unitType: UnitType.pieces,
          inputType: FieldInputType.slider,
          defaultValue: 1.0,
          minValue: 0.0,
          maxValue: 5.0,
          step: 1.0,
          iconName: 'door_sliding',
          group: 'options',
          required: false,
          order: 4,
        ),
        CalculatorField(
          key: 'wicketsCount',
          labelKey: 'input.wickets',
          unitType: UnitType.pieces,
          inputType: FieldInputType.slider,
          defaultValue: 1.0,
          minValue: 0.0,
          maxValue: 5.0,
          step: 1.0,
          iconName: 'door_front',
          group: 'options',
          required: false,
          order: 5,
        ),
        CalculatorField(
          key: 'sheetWorkingWidthMm',
          labelKey: 'input.fence.sheet_working_width',
          hintKey: 'input.fence.sheet_working_width.hint',
          unitType: UnitType.millimeters,
          inputType: FieldInputType.number,
          defaultValue: 1150,
          minValue: 500,
          maxValue: 1500,
          step: 10,
          group: 'material',
          order: 6,
        ),
        CalculatorField(
          key: 'coverReservePercent',
          labelKey: 'input.fence.cover_reserve',
          unitType: UnitType.percent,
          inputType: FieldInputType.number,
          defaultValue: 0,
          minValue: 0,
          maxValue: 30,
          step: 1,
          group: 'material',
          order: 7,
        ),
        CalculatorField(
          key: 'screwsPerSheet',
          labelKey: 'input.fence.screws_per_sheet',
          hintKey: 'input.fence.screws_per_sheet.hint',
          unitType: UnitType.pieces,
          inputType: FieldInputType.number,
          defaultValue: 6,
          minValue: 0,
          maxValue: 30,
          step: 1,
          group: 'fasteners',
          order: 8,
        ),
        CalculatorField(
          key: 'screwReservePercent',
          labelKey: 'input.fence.screw_reserve',
          unitType: UnitType.percent,
          inputType: FieldInputType.number,
          defaultValue: 5,
          minValue: 0,
          maxValue: 30,
          step: 1,
          group: 'fasteners',
          order: 9,
        ),
        CalculatorField(
          key: 'screwPackCount',
          labelKey: 'input.fence.screw_pack',
          unitType: UnitType.pieces,
          inputType: FieldInputType.number,
          defaultValue: 200,
          minValue: 1,
          maxValue: 5000,
          step: 10,
          group: 'fasteners',
          order: 10,
        ),
      ],
      beforeHints: [
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.stolby_ustanavlivayutsya_na_glubinu'),
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.dlya_proflista_ispolzuyte_otsinkovannye'),
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.derevyannyy_zabor_trebuet_obrabotki'),
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.kirpichnyy_zabor_nuzhdaetsya_v'),
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.rasstoyanie_mezhdu_stolbami_2'),
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.vorota_i_kalitki_dolzhny'),
      ],
      afterHints: [
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.stolby_ustanavlivayutsya_na_glubinu'),
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.dlya_proflista_ispolzuyte_otsinkovannye'),
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.derevyannyy_zabor_trebuet_obrabotki'),
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.kirpichnyy_zabor_nuzhdaetsya_v'),
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.rasstoyanie_mezhdu_stolbami_2'),
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.vorota_i_kalitki_dolzhny'),
      ],
      useCase: CanonicalBridgeUseCase(calculateCanonicalFence),
      accentColor: kCalculatorAccentColor,
      complexity: 3,
      popularity: 10,
      tags: [
        'tag.konstruktsii',
        'tag.zabory',
        'fence',
      ],
    ),
  const CalculatorDefinitionV2(
      id: 'stairs',
      titleKey: 'calculator.stairs.title',
      descriptionKey: 'calculator.stairs.description',
      category: CalculatorCategory.exterior,
      subCategoryKey: 'subcategory.stairs',
      fields: [
        CalculatorField(
          key: 'floorHeight',
          labelKey: 'input.floorHeight',
          unitType: UnitType.meters,
          inputType: FieldInputType.slider,
          defaultValue: 3.0,
          minValue: 2.0,
          maxValue: 6.0,
          step: 1.0,
          iconName: 'height',
          group: 'dimensions',
          required: true,
          order: 1,
        ),
        CalculatorField(
          key: 'stepHeight',
          labelKey: 'input.stepHeight',
          unitType: UnitType.meters,
          inputType: FieldInputType.slider,
          defaultValue: 0.18,
          minValue: 0.1,
          maxValue: 0.3,
          step: 0.01,
          iconName: 'height',
          group: 'dimensions',
          required: true,
          order: 2,
        ),
        CalculatorField(
          key: 'stepWidth',
          labelKey: 'input.stepWidth',
          unitType: UnitType.meters,
          inputType: FieldInputType.slider,
          defaultValue: 0.28,
          minValue: 0.2,
          maxValue: 0.4,
          step: 0.01,
          iconName: 'width_normal',
          group: 'dimensions',
          required: true,
          order: 3,
        ),
        CalculatorField(
          key: 'stepCount',
          labelKey: 'input.stepCount',
          unitType: UnitType.pieces,
          inputType: FieldInputType.slider,
          defaultValue: 0.0,
          minValue: 0.0,
          maxValue: 50.0,
          step: 1.0,
          iconName: 'format_list_numbered',
          group: 'dimensions',
          required: false,
          order: 4,
        ),
        CalculatorField(
          key: 'width',
          labelKey: 'input.width',
          unitType: UnitType.meters,
          inputType: FieldInputType.slider,
          defaultValue: 1.0,
          minValue: 0.6,
          maxValue: 2.0,
          step: 1.0,
          iconName: 'straighten',
          group: 'dimensions',
          required: true,
          order: 5,
        ),
        CalculatorField(
          key: 'materialType',
          labelKey: 'input.type',
          unitType: UnitType.pieces,
          inputType: FieldInputType.number,
          defaultValue: 1.0,
          minValue: 1.0,
          maxValue: 3.0,
          iconName: 'category',
          group: 'material',
          required: true,
          order: 6,
        ),
      ],
      beforeHints: [
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.vysota_stupeni_dolzhna_byt'),
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.shirina_prostupi_stupeni_dolzhna'),
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.shirina_lestnitsy_dlya_zhilyh'),
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.dlya_derevyannoy_lestnitsy_ispolzuyte'),
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.betonnaya_lestnitsa_trebuet_armirovaniya'),
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.perila_dolzhny_byt_na'),
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.balyasiny_ustanavlivayutsya_s_shagom'),
      ],
      afterHints: [
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.vysota_stupeni_dolzhna_byt'),
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.shirina_prostupi_stupeni_dolzhna'),
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.shirina_lestnitsy_dlya_zhilyh'),
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.dlya_derevyannoy_lestnitsy_ispolzuyte'),
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.betonnaya_lestnitsa_trebuet_armirovaniya'),
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.perila_dolzhny_byt_na'),
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.balyasiny_ustanavlivayutsya_s_shagom'),
      ],
      useCase: CanonicalBridgeUseCase(calculateCanonicalStairs),
      accentColor: kCalculatorAccentColor,
      complexity: 3,
      popularity: 10,
      tags: [
        'tag.konstruktsii',
        'stairs',
        'tag.lestnitsy',
      ],
    ),
  const CalculatorDefinitionV2(
      id: 'terrace',
      titleKey: 'calculator.terrace.title',
      descriptionKey: 'calculator.terrace.description',
      category: CalculatorCategory.exterior,
      subCategoryKey: 'subcategory.terrace_veranda',
      fields: [
        CalculatorField(
          key: 'length',
          labelKey: 'input.terrace.length',
          unitType: UnitType.meters,
          inputType: FieldInputType.slider,
          defaultValue: 5,
          minValue: 1,
          maxValue: 30,
          step: 0.5,
          iconName: 'straighten',
          group: 'dimensions',
          order: 1,
        ),
        CalculatorField(
          key: 'width',
          labelKey: 'input.terrace.width',
          unitType: UnitType.meters,
          inputType: FieldInputType.slider,
          defaultValue: 3,
          minValue: 1,
          maxValue: 15,
          step: 0.5,
          iconName: 'width_normal',
          group: 'dimensions',
          order: 2,
        ),
        CalculatorField(
          key: 'boardType',
          labelKey: 'input.terrace.board_type',
          unitType: UnitType.pieces,
          inputType: FieldInputType.select,
          defaultValue: 0,
          minValue: 0,
          maxValue: 3,
          iconName: 'deck',
          group: 'material',
          order: 3,
          options: [
            FieldOption(value: 0, labelKey: 'input.terrace.board_type.wpc'),
            FieldOption(value: 1, labelKey: 'input.terrace.board_type.larch'),
            FieldOption(value: 2, labelKey: 'input.terrace.board_type.pine'),
            FieldOption(value: 3, labelKey: 'input.terrace.board_type.planken'),
          ],
        ),
        CalculatorField(
          key: 'boardLength',
          labelKey: 'input.terrace.board_length',
          unitType: UnitType.millimeters,
          inputType: FieldInputType.number,
          defaultValue: 3000,
          minValue: 1000,
          maxValue: 12000,
          step: 100,
          group: 'material',
          order: 4,
        ),
        CalculatorField(
          key: 'boardWidthMm',
          labelKey: 'input.terrace.board_width',
          hintKey: 'input.terrace.board_width.hint',
          unitType: UnitType.millimeters,
          inputType: FieldInputType.number,
          defaultValue: 150,
          minValue: 70,
          maxValue: 300,
          step: 1,
          group: 'material',
          order: 5,
        ),
        CalculatorField(
          key: 'gapMm',
          labelKey: 'input.terrace.gap',
          hintKey: 'input.terrace.gap.hint',
          unitType: UnitType.millimeters,
          inputType: FieldInputType.number,
          defaultValue: 5,
          minValue: 0,
          maxValue: 20,
          step: 1,
          group: 'material',
          order: 6,
        ),
        CalculatorField(
          key: 'offcutReuseMode',
          labelKey: 'input.terrace.offcut_reuse',
          hintKey: 'input.terrace.offcut_reuse.hint',
          unitType: UnitType.pieces,
          inputType: FieldInputType.switch_,
          defaultValue: 0,
          minValue: 0,
          maxValue: 1,
          group: 'layout',
          order: 7,
        ),
        CalculatorField(
          key: 'boardReservePercent',
          labelKey: 'input.terrace.board_reserve',
          unitType: UnitType.percent,
          inputType: FieldInputType.number,
          defaultValue: 10,
          minValue: 0,
          maxValue: 30,
          step: 1,
          group: 'layout',
          order: 8,
        ),
        CalculatorField(
          key: 'lagStep',
          labelKey: 'input.terrace.lag_step',
          hintKey: 'input.terrace.lag_step.hint',
          unitType: UnitType.millimeters,
          inputType: FieldInputType.select,
          defaultValue: 400,
          minValue: 200,
          maxValue: 1000,
          group: 'frame',
          order: 9,
          options: [
            FieldOption(value: 300, labelKey: 'input.terrace.lag_step.300'),
            FieldOption(value: 400, labelKey: 'input.terrace.lag_step.400'),
            FieldOption(value: 500, labelKey: 'input.terrace.lag_step.500'),
            FieldOption(value: 600, labelKey: 'input.terrace.lag_step.600'),
          ],
        ),
        CalculatorField(
          key: 'lagLengthM',
          labelKey: 'input.terrace.lag_length',
          unitType: UnitType.meters,
          inputType: FieldInputType.number,
          defaultValue: 3,
          minValue: 1,
          maxValue: 12,
          step: 0.5,
          group: 'frame',
          order: 10,
        ),
        CalculatorField(
          key: 'lagReservePercent',
          labelKey: 'input.terrace.lag_reserve',
          unitType: UnitType.percent,
          inputType: FieldInputType.number,
          defaultValue: 5,
          minValue: 0,
          maxValue: 30,
          step: 1,
          group: 'frame',
          order: 11,
        ),
        CalculatorField(
          key: 'clipsPerIntersection',
          labelKey: 'input.terrace.clips_per_intersection',
          hintKey: 'input.terrace.clips_per_intersection.hint',
          unitType: UnitType.pieces,
          inputType: FieldInputType.number,
          defaultValue: 1,
          minValue: 0,
          maxValue: 4,
          step: 1,
          group: 'fasteners',
          order: 12,
        ),
        CalculatorField(
          key: 'starterClipsPerRow',
          labelKey: 'input.terrace.starter_clips',
          unitType: UnitType.pieces,
          inputType: FieldInputType.number,
          defaultValue: 2,
          minValue: 0,
          maxValue: 4,
          step: 1,
          group: 'fasteners',
          order: 13,
        ),
        CalculatorField(
          key: 'clipPackCount',
          labelKey: 'input.terrace.clip_pack',
          unitType: UnitType.pieces,
          inputType: FieldInputType.number,
          defaultValue: 100,
          minValue: 1,
          maxValue: 1000,
          step: 1,
          group: 'fasteners',
          order: 14,
        ),
        CalculatorField(
          key: 'fastenersPerClip',
          labelKey: 'input.terrace.fasteners_per_clip',
          unitType: UnitType.pieces,
          inputType: FieldInputType.number,
          defaultValue: 1,
          minValue: 0,
          maxValue: 4,
          step: 1,
          group: 'fasteners',
          order: 15,
        ),
        CalculatorField(
          key: 'fastenerPackCount',
          labelKey: 'input.terrace.fastener_pack',
          unitType: UnitType.pieces,
          inputType: FieldInputType.number,
          defaultValue: 100,
          minValue: 1,
          maxValue: 5000,
          step: 10,
          group: 'fasteners',
          order: 16,
        ),
        CalculatorField(
          key: 'fastenerReservePercent',
          labelKey: 'input.terrace.fastener_reserve',
          unitType: UnitType.percent,
          inputType: FieldInputType.number,
          defaultValue: 5,
          minValue: 0,
          maxValue: 30,
          step: 1,
          group: 'fasteners',
          order: 17,
        ),
        CalculatorField(
          key: 'withTreatment',
          labelKey: 'input.terrace.treatment',
          unitType: UnitType.pieces,
          inputType: FieldInputType.select,
          defaultValue: 0,
          minValue: 0,
          maxValue: 2,
          group: 'treatment',
          order: 18,
          options: [
            FieldOption(value: 0, labelKey: 'input.terrace.treatment.none'),
            FieldOption(value: 1, labelKey: 'input.terrace.treatment.oil'),
            FieldOption(value: 2, labelKey: 'input.terrace.treatment.antiseptic'),
          ],
        ),
        CalculatorField(
          key: 'treatmentRateLPerM2PerLayer',
          labelKey: 'input.terrace.treatment_rate',
          hintKey: 'input.terrace.treatment_rate.hint',
          unitType: UnitType.litersPerSqm,
          inputType: FieldInputType.number,
          defaultValue: 0.1,
          minValue: 0.01,
          maxValue: 1,
          step: 0.01,
          group: 'treatment',
          order: 19,
          dependency: FieldDependency(
            fieldKey: 'withTreatment',
            condition: DependencyCondition.greaterThan,
            value: 0,
          ),
        ),
        CalculatorField(
          key: 'treatmentLayers',
          labelKey: 'input.terrace.treatment_layers',
          unitType: UnitType.pieces,
          inputType: FieldInputType.number,
          defaultValue: 2,
          minValue: 1,
          maxValue: 4,
          step: 1,
          group: 'treatment',
          order: 20,
          dependency: FieldDependency(
            fieldKey: 'withTreatment',
            condition: DependencyCondition.greaterThan,
            value: 0,
          ),
        ),
        CalculatorField(
          key: 'treatmentCanL',
          labelKey: 'input.terrace.treatment_can',
          unitType: UnitType.liters,
          inputType: FieldInputType.number,
          defaultValue: 2.5,
          minValue: 0.5,
          maxValue: 20,
          step: 0.5,
          group: 'treatment',
          order: 21,
          dependency: FieldDependency(
            fieldKey: 'withTreatment',
            condition: DependencyCondition.greaterThan,
            value: 0,
          ),
        ),
        CalculatorField(
          key: 'treatmentReservePercent',
          labelKey: 'input.terrace.treatment_reserve',
          unitType: UnitType.percent,
          inputType: FieldInputType.number,
          defaultValue: 10,
          minValue: 0,
          maxValue: 30,
          step: 1,
          group: 'treatment',
          order: 22,
          dependency: FieldDependency(
            fieldKey: 'withTreatment',
            condition: DependencyCondition.greaterThan,
            value: 0,
          ),
        ),
        CalculatorField(
          key: 'withGeotextile',
          labelKey: 'input.terrace.with_geotextile',
          unitType: UnitType.pieces,
          inputType: FieldInputType.switch_,
          defaultValue: 1,
          minValue: 0,
          maxValue: 1,
          group: 'preparation',
          order: 23,
        ),
        CalculatorField(
          key: 'geotextileRollM2',
          labelKey: 'input.terrace.geotextile_roll',
          unitType: UnitType.squareMeters,
          inputType: FieldInputType.number,
          defaultValue: 50,
          minValue: 5,
          maxValue: 200,
          step: 5,
          group: 'preparation',
          order: 24,
          dependency: FieldDependency(
            fieldKey: 'withGeotextile',
            condition: DependencyCondition.equals,
            value: 1,
          ),
        ),
        CalculatorField(
          key: 'geotextileReservePercent',
          labelKey: 'input.terrace.geotextile_reserve',
          unitType: UnitType.percent,
          inputType: FieldInputType.number,
          defaultValue: 5,
          minValue: 0,
          maxValue: 30,
          step: 1,
          group: 'preparation',
          order: 25,
          dependency: FieldDependency(
            fieldKey: 'withGeotextile',
            condition: DependencyCondition.equals,
            value: 1,
          ),
        ),
      ],
      beforeHints: [
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.terrasnaya_doska_deking_ustoychiva'),
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.plitka_dlya_terrasy_dolzhna'),
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.ograzhdenie_obespechivaet_bezopasnost_osobenno'),
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.krovlya_zaschischaet_ot_dozhdya'),
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.polikarbonat_propuskaet_svet_i'),
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.stolby_dlya_krovli_dolzhny'),
      ],
      afterHints: [
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.terrasnaya_doska_deking_ustoychiva'),
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.plitka_dlya_terrasy_dolzhna'),
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.ograzhdenie_obespechivaet_bezopasnost_osobenno'),
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.krovlya_zaschischaet_ot_dozhdya'),
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.polikarbonat_propuskaet_svet_i'),
        CalculatorHint(type: HintType.tip, messageKey: 'hint.facade.stolby_dlya_krovli_dolzhny'),
      ],
      useCase: CanonicalBridgeUseCase(calculateCanonicalTerrace),
      accentColor: kCalculatorAccentColor,
      complexity: 3,
      popularity: 10,
      tags: [
        'terrace',
        'tag.konstruktsii',
        'tag.terrasa_veranda',
      ],
    ),
  // insulation_mineral_wool удалён — UI не готов
];
