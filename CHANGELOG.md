...
### Исправлено
- **Михалыч снова показывает ответы**: Flutter-клиент понимает новый agent SSE-контракт сервера и сохраняет совместимость со старым OpenAI-форматом.
- **Площадь штукатурки больше не выглядит противоречиво**: в ручном режиме явно показан вычет площади окон и дверей, а результат подписан как итоговая площадь под штукатурку.
- **Расчёты и списки покупок синхронизированы с сайтом**: устранены все расхождения в 70 эталонных сценариях — включая утепление, арматуру, плитку, бетон, фасады, полы, окна и инженерные системы.
- **Режимы точности снова влияют на результат корректно**: «Базовый», «Реальный» и «Профессиональный» применяют те же строительные коэффициенты, что и web-калькуляторы, в том числе к сопутствующим материалам.
- **Упаковки считаются до реальной покупки**: плитка округляется до полных коробок, утеплитель — до упаковок, кассетный потолок — до пачек, арматура — до стандартных прутков.
- **Списки материалов стали полнее и понятнее**: добавлены недостающие крепёж, грунтовки, профили, заглушки, герметики и расходники; названия приведены к единому виду.
- **Арматура больше не смешивает килограммы и погонные метры**: точная длина, число прутков, остаток и масса теперь показываются в своих единицах.

### Релиз
- Версия выпуска сверяется с `pubspec.yaml` до создания GitHub Release.
- Release signing и Firebase-конфиг подготавливаются из GitHub Secrets, как в основном CI.
- GitHub Release остаётся черновиком до успешных analyze, test, APK- и AAB-сборок.

## [1.7.1] - 2026-05-25

### Добавлено
- **5 новых калькуляторов для участка и наружных систем** — с полноценными кастомными экранами:
  - **Газон**: посевной или рулонный, тип грунта (песчаный/суглинок/глинистый), интенсивность использования (декоративный/обычный/спортивный), опции дренажа и геотекстиля
  - **Дренаж участка**: типы (линейный/кольцевой/сложный), диаметр трубы Ø110/Ø160, уровень грунтовых вод, опция приёмного колодца
  - **Теплица**: арочная или двускатная крыша, размер, толщина поликарбоната 4/6/8/10 мм, тип фундамента
  - **Тротуарная плитка**: основание (песок/ЦПС/бетон), толщина 30/40/60/80 мм, бордюр
  - **Септик из колец**: количество камер 1–3, диаметр колец 1000/1500/2000 мм, тип грунта, фильтрующий колодец
- Smoke-тесты для всех 5 новых экранов

### Исправлено
- **Критичный баг полей ввода**: при вводе числа поле сбрасывалось на старое значение и блокировало редактирование. Виной двойной `setState` (внутри `onChanged` + внутри `_update()`) и отсутствие правильной работы с TextEditingController. `CalculatorTextField` теперь уведомляет родителя только на потерю фокуса / коммит, что полностью устраняет проблему. Затронуло 47 экранов калькуляторов
- 14 упавших widget-тестов (`CalculatorScaffold`, `CalculatorEngine`)
- 5 ошибок компиляции в `share_project_usecase_test` (required `labels`)

### Изменено
- `ProCalculatorScreen` теперь рендерит `CanonicalMaterialResult` через `CanonicalMaterialsCard` — единый стиль материалов для всех калькуляторов
- Унификация всех экранов через `CalculatorEngine` (расчёт идёт из одного места)

### Удалено
- 10 мёртвых use cases, не используемых ни одним экраном (заменены v2-версиями)
- 40 unused_import после рефакторинга на `CalculatorEngine`

### Качество
- `flutter analyze`: 0 errors, 0 warnings
- `flutter test`: 5446 passed, 0 failed
- `dart fix --apply`: 98 косметических фиксов
